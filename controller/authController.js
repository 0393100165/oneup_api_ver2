...
module.exports = {
    ...
    DangKyAccount: async function (req, res) {
        ...
        if (trungTaiKhoan === null) {
            ...
            const emailData = {
                ...
                to: userThem.email,
                subject: 'Kích hoạt tài khoản OneUp',
                html: `
            ...
            await sgMail.send(emailData).then(sent => {
                ...
                res.status(200).json({
                    status: 'fail',
                    message: 'Đăng ký tài khoản bị lỗi'
                })
            })
        } else {
            res.status(200).json({
    ...
    QuenPassword: async function (req, res) {
        ...
        if (trungTaiKhoan === null) {
            res.status(200).json({
                status: 'fail',
                message: 'Email này không tồn tại trong hệ thống OneUp',
            })
        } else {
            ...
            const emailData = {
                from: process.env.EMAIL_FROM,
                to: userEmail.email,
                subject: 'Xác nhận đổi mật khẩu tài khoản OneUp',
                html: `
                    <h1>Vui lòng nhấn vào link dưới đây để đổi mật khẩu tài khoản OneUp</h1>
